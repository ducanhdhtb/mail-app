...
export class DataService {
  ...
  getUniqueFolders(): string[] {
    // 1 [ {f: 'inbox'}, {f: 'inbox'}, {f: spam}]
    // 2 ['inbox', 'inbox', 'spam']
    // 3 [inbox, spam]

    const folders = this.data.map(m => m.folder); // => 2
    const save = new Map();
    const res = [];
  ...
  getMessagesByFolder(f: string): Message[] {
    return this.data.filter(m => m.folder === f);
  }

  findById(id: string): Message {
    return this.data.find(m => m._id === id);
  }
}
